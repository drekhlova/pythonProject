''' Чистые функции (Pure Functions):  Они не изменяют
состояние программы или внешние переменные.
'''
def add(a, b):
    return a + b
...
''' Неизменяемость (Immutable Data):  после 
создания объекта его нельзя изменить. Вместо 
этого создается новый объект с измененным состоянием.'''
...
Это функции, которые принимают другие функции в качестве 
аргументов или возвращают их в качестве результатов.'''
def apply(func, x):
    return func(x)
...
''' Рекурсия: Функциональное программирование активно использует 
...
''' Функции первого класса (First-Class Functions): Функции 
рассматриваются как обычные значения и могут быть присвоены 
